...
kony.mvc.FormController = function () {
	...
	FormController.prototype.getPreviousForm = function () {
		...
		if (null != prevForm) {
			if (prevForm._konyControllerName)
				return prevForm.id;
			else
				return prevForm
		}
	...
	FormController.prototype.getCurrentForm = function () {
		...
		if (null != currForm) {
			if (currForm._konyControllerName)
				return currForm.id;
			else
				return currForm
		}
...
kony.application.destroyForm = function (formFriendlyName) {
	...
	if (null != formID) {
		if (formID in _kony.mvc.viewId2ControllerNameMap) {
			...
			require.undef("user"+tmpControllerName);
			require.undef(formID);
		}
	}
};
			...
			for (var key in controllers[i]) {
				if (i < controllers.length && _kony.mvc.isValidControllerKey(key)) {
					if (typeof controllers[i][key] === "function") {
						...
						if (i == 0)
							tmpController["parent"][key] = controllers[i][key].bind(tmpController)
					} else {
						tmpController[extensionLevel][key] = controllers[i][key];
						if (i == 0)
							tmpController["parent"][key] = controllers[i][key]
					}
				}
...
_kony.mvc.setMasterContract4SPA = function (userWidgetInstance) {
	var config = {};
	if (null == userWidgetInstance || userWidgetInstance._konyInitialized)
		return;
...
_kony.mvc.setProperties = function (userWidgetInstance, properties, tmpController) {
	if (null == properties || null == userWidgetInstance || null == tmpController)
		return;
...
_kony.mvc.setFunctions = function (userWidgetInstance, lstAPIs, tmpController, isView) {
	if (null == lstAPIs || null == userWidgetInstance || null == tmpController)
		return;
...
_kony.mvc.setEvents = function (userWidgetInstance, lstEvents, tmpController) {
	if (null == lstEvents || null == userWidgetInstance || null == tmpController)
		return;
...
kony.mvc.Navigation = function () {
	function Navigation(formname, objModel) {
		...
		this.navigate = function (param) {
			...
			if (null == model) {
				if (controller.getModel)
					model = controller.getModel.call(controller)
			}
			if (controller.setModel)
				controller.setModel.call(controller, model);
...
kony.mvc.TemplateController = function (viewId1) {
	...
	this.getCurrentView = function (childWidget) {
		while (childWidget.parent != null) {
			if (childWidget.id == this.view.id)
				break;
...
_kony = _kony || {};
_kony.mvc = _kony.mvc || {};
if (kony.mvc == undefined)
	kony.mvc = {};
if (kony.mvc.registry == undefined)
	kony.mvc.registry = {};
...
kony.model.Util.perftimecal = function (startTag, endTag, startTS, endTS) {
	if (kony.model.Util.isPerfTestReq === false)
		return;
...
kony.model.DataStore = function () {
	function DataStore() {
		...
		this.getDataByKey = function (key) {
			if (inMemoryData.hasOwnProperty(key))
...
kony.model.AppFactory = function () {
	...
	AppFactory.prototype.createAuthenticationManager = function () {
		...
		if (kony.model.ApplicationContext)
			kony.model.ApplicationContext.setAuthManager(authManager);
		return authManager
	};
	AppFactory.prototype.getAuthManager = function () {
		if (kony.model.ApplicationContext && kony.model.ApplicationContext.getAuthManager())
			return kony.model.ApplicationContext.getAuthManager();
		else
			return this.createAuthManager()
	};
...
kony.model.ApplicationContext = function () {
	...
	ApplicationContext.prototype.getMetadataStore = function () {
		if (!this.metadataStore)
			this.metadataStore = new kony.model.MetadataStore;
		return this.metadataStore
	};
	ApplicationContext.prototype.getSyncManager = function () {
		if (!this.syncManager)
			this.syncManager = this.getFactorySharedInstance().createSyncManagerObject();
	...
	ApplicationContext.prototype.setConfigParams = function (params) {
		if (this.configParams) {
			for (var key in params)
				this.configParams[key] = params[key]
		} else
	...
	ApplicationContext.prototype.initializeObjectServices = function (serviceEntityMap, successCallback, errorCallback) {
		...
		function initialiseServices() {
			...
			function initialiseObject() {
				...
				if (!metadataOptions)
					metadataOptions = serviceMetadataOptions;
				if (metadataOptions.getFromServer) {
					var model = serviceName + "." + entityName;
...
kony.model.BaseModel = function () {
	function BaseModel(applicationContext, entityMetaData, configOptions) {
		...
		this.getAccessType = function () {
			if (options && options.hasOwnProperty("access") && options.access === "offline")
				return "offline";
			else
				return "online"
		};
	...
	BaseModel.prototype.fetchDataForColumns = function (columnNames, onSuccess, onError, dataModel) {
		try {
			...
			if (this.getAccessType() === "offline")
				dataObject = this.getDataObjectOffline(columnNames, dataModel);
			else
				dataObject = this.getDataObjectOnline(columnNames, dataModel);
	...
	BaseModel.prototype.fetchResponse = function (options, onSuccess, onError) {
		...
		try {
			...
			var exception;
			if (e instanceof kony.model.Exception)
				exception = self.getApplicationContext().getFactorySharedInstance().createExceptionObject(e.code, e.message, e);
			else
				exception = self.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.model.ExceptionCode.CD_ERROR_FETCH, kony.model.ExceptionCode.MSG_ERROR_FETCH, e);
			onError(exception)
		}
		function success(response) {
	...
	BaseModel.prototype.customVerb = function (verbName, options, success, error) {
		...
		try {
			...
			if (e instanceof kony.model.Exception)
				...
				exception = self.getApplicationContext().getFactorySharedInstance().createExceptionObject(kony.model.ExceptionCode.CD_ERROR_CUSTOMVERB, kony.model.ExceptionCode.MSG_ERROR_CUSTOMVERB, e);
			onError(exception)
		}
	...
	MetadataServiceManagerMF.prototype.fetchForAllObjectServices = function (options, metadataOptions, success, error) {
		...
		function getAllObjectServicesFromSdk() {
			...
			if (kony.sdk.registeredobjsvcs)
				if (objectServicesMapFromSdk)
					objectServicesMapFromSdk = kony.model.Util.mergeJSONs(objectServicesMapFromSdk, kony.sdk.registeredobjsvcs);
				else
					objectServicesMapFromSdk = kony.sdk.registeredobjsvcs;
...
kony.model.MetadataStore = function () {
	...
	MetadataStore.prototype.getEntityMetadata = function (entity, serviceName, options) {
		...
		if (svcMetadata && svcMetadata[entity])
			return svcMetadata[entity]
	};
...
kony.model.logger = function (logLevel, tag, msg, params) {
	if (logLevel.value <= kony.model.currentLogLevel.value) {
		...
		if (kony.model.Util.isValidJs(params)) {
			params = kony.model.Util.stringifyKonyObject(params)
		}
		var date = (new Date).toLocaleDateString();
		var time = (new Date).toLocaleTimeString();
		var level = logLevel.code;
		var formattedMessage = "[" + date + "][" + time + "][" + tag + "][" + level + "] : " + msg + " " + params;
		if (kony.model.error_alert && logLevel.value == kony.model.log.ERROR.value)
...
kony.model.Exception = function () {
	function toStringRecursive(exception, output) {
		...
		if (exception.getErrorObj()) {
			if (exception.getErrorObj().name && exception.getErrorObj().message)
				output.append(exception.getErrorObj().name + ": " + exception.getErrorObj().message + ": " + exception.getErrorObj().sourceURL);
			else
				output.append(JSON.stringify(exception.getErrorObj()));
			output.append("\n");
	...
	Exception.prototype.alert = function (full) {
		if (full === true) {
			...
			alert(this.code + ":" + this.name + ":" + this.message)
	};
