 ...
 */
/*global block*/
var fs = require('fs'),beautify = require('./beautify.js')
...
var inputdir = process.argv[2],outputdir = process.argv[3],platforms = process.argv[4].split(","),defines = {};

console.log(inputdir);
//console.log(platforms);
platforms.forEach(function(platform){
	defines[platform] = true;
})
...
fs.readdir(inputdir,function(err,files) {
	...
	files.forEach(function(file) {
		if (file.indexOf(".js") != -1) {
			...
			fs.writeFileSync(outputdir+"/"+file,beautify.js_beautify(preprocess(fs.readFileSync(inputdir+"/"+file,'utf-8'),defines).join('\n'),{preserve_newlines:false}));
		}
	},this);
